...
% See also stat_summary(), stat_bin(), stat_density()
    ...
    my_addParameter(p,'wmax',1);

    my_addParameter(p,'bwfactor',[])
    my_addParameter(p,'ksdensityParam',[]);
    parse(p,varargin{:});
...
for ind_x=1:length(uni_x)
    ...
    if params.histbin == 1
        [zN,binranges] = histcounts(ysel,params.npoints);
    elseif params.histbin == 2
        [zN,binranges] = histcounts(ysel);
    else        
        extra_y=(max(ysel)-min(ysel))*params.extra_y;    
        binranges=linspace(min(ysel)-extra_y,max(ysel)+extra_y);
    end
    if isempty(params.ksdensityParam)
        if params.bandwidth>0
            [dens{ind_x},dens_pos{ind_x},bwP(ind_x)] = ksdensity(ysel,binranges,'function','pdf',...
            ...
            if ~isempty(params.bwfactor) && params.bwfactor ~= 1
                
                [dens{ind_x},dens_pos{ind_x},bwP(ind_x)] = ksdensity(ysel,binranges,'function','pdf',...
                    'kernel',params.kernel,'bandwidth',params.bwfactor*bwP(ind_x));

            end
        end    
    else 
        useNNZ = 0;
        kParam = params.ksdensityParam;
        if isfield(kParam,'useNNZ') 
            useNNZ = kParam.useNNZ;
            kParam = rmfield(kParam,'useNNZ');
        end

        zparam = [ fieldnames(kParam) struct2cell(kParam) ]';
        if useNNZ
            fprintf('Using custom params - NNZ only (%d val)',nnz(ysel));

            [dens{ind_x},dens_pos{ind_x},bwP(ind_x)] = ksdensity(nonzeros(ysel),zparam{:})
        else
            [dens{ind_x},dens_pos{ind_x},bwP(ind_x)] = ksdensity(ysel,zparam{:})
        end
    end
...
switch params.normalization
    case 'maxScale'
        ...
        dy = [ dens{:} ];
        mxdense = max(dy);
        yfact = (params.wmax/2)/mxdense;
        for ind_x=1:length(uni_x)
            dens{ind_x} = dens{ind_x}*yfact;
        end       
        
    case 'equalWidth'
        %dy = vertcat(dens{:})';
        mxdense = cellfun(@(x)max(x),dens);
        yfact = (params.wmax/2)./mxdense;
        for ind_x=1:length(uni_x)
            dens{ind_x} = dens{ind_x}*yfact(ind_x);
        end 
end
