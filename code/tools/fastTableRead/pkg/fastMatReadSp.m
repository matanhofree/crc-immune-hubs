...
if strcmp(computer,'MACI64')
    ...
    if nargout > 2
        if isempty(commentLine)
            [zdata,zerr,zRowH,zColH] = fastTableReadV3(3,fname,[],delim,skipRows,headerRows,headerCols);
        else
            [zdata,zerr,zRowH,zColH] = fastTableReadV3(3,fname,[],delim,skipRows,headerRows,headerCols,commentLine);
        end
    else
        if isempty(commentLine)
            [zdata,zerr] = fastTableReadV3(3,fname,[],delim,skipRows,headerRows,headerCols);
        else
            [zdata,zerr] = fastTableReadV3(3,fname,[],delim,skipRows,headerRows,headerCols,commentLine);
        end
    end
else
